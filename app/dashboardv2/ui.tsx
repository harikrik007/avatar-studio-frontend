...
// Copied from the v1 dashboard rather than shared with it: v1 is the
// Wav2Lip product and will change on its own schedule (or go away), and a
// helper both dashboards import would tie them together exactly where
...
export function AvatarThumb({
  ...
  // A still wins when there is one: an Anam avatar's preview is an image
  // on their CDN, and an <img> costs nothing next to an autoplaying clip.
  if (imageSrc) {
    return <img className="l-avatar-thumb" src={imageSrc} alt="" />;
  }
  if (src) {
